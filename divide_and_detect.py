...
def divide_and_detect(path1, path2, destination_path, store_image, model_path): 
    dev = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    model = torch.load(model_path, map=dev)
    ...
    for i in range(height_num):
        ...
        for j in range(width_num):
            ...
            window_diff_map = detect_change(None, None, None, False, window_image1, window_image2, model, None)
        ...
        if image1.shape[1] != 256*width_num:
            ...
            small_window_diff_map = detect_change(None, None, None, False, small_window_image1, small_window_image2, model, None)
...
if __name__ == '__main__':
    divide_and_detect(path1, path2, destination_path, True, model_path)
