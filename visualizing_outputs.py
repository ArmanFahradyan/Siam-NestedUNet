...
for name in _names:
    ...
    if name == _names[0]:
        assert len(args.lsit_paths) == len(args.list_names), "inappropriate number of methods or names"
        num_methods = len(args.lsit_paths)
        img_size = img1.shape[0]
        row_count = (len(args.lsit_paths)+2) // 3
        col_stick = 123*np.ones((img_size, 5, 3))
        row_stick = 123*np.ones((5, 3*img_size + 2*5, 3))

    preds = []
    for i, method in enumerate(args.list_names):
        pred = cv2.imread(args.list_paths + name)[:, img_size:]
